...
def load_adult():
    train_df = pd.read_csv(TRAIN_PATH, header=None, names=ADULT_COLUMNS, skipinitialspace=True, na_values='?')
    test_df = pd.read_csv(TEST_PATH, header=None, names=ADULT_COLUMNS, skipinitialspace=True, na_values='?', comment='|')
    test_df['income'] = test_df['income'].astype(str).str.replace('.', '', regex=False)
    train_df = train_df.dropna(subset=['income']); test_df = test_df.dropna(subset=['income'])
    return train_df, test_df


def build_and_fit_pipeline(train_df: pd.DataFrame):
    X = train_df.drop(columns=[LABEL]); y = (train_df[LABEL] == '>50K').astype(int)
...
def get_feature_schema(model, num_cols, cat_cols):
    ...
    for col, cats in zip(cat_cols, ohe.categories_):
        cat_group_ranges[col] = {'start': offset, 'end': offset + len(cats), 'categories': list(map(str, cats))}
        offset += len(cats)
...
def main():
    ...
    rules = clf.rules_; default_class = clf.default_class_
    ...
    for col, meta in numeric_meta.items():
        i = meta['index']
        s.add(x_vars[i] >= RealVal(float(mins[i]))); s.add(x_vars[i] <= RealVal(float(maxs[i])))
        s.add(y_vars[i] >= RealVal(float(mins[i]))); s.add(y_vars[i] <= RealVal(float(maxs[i])))
