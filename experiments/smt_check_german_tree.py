...
def tree_pred_expr(tree, zvars, node_id=0):
    left = tree.children_left[node_id]; right = tree.children_right[node_id]
    if left == right:
        value = tree.value[node_id][0]; pred_cls = int(np.argmax(value)); return RealVal(pred_cls)
...
def main():
    ...
    x_vars = [Real(f'x_{i}') for i in range(n_features)]; y_vars = [Real(f'y_{i}') for i in range(n_features)]
    s = Solver()
    for col, meta in numeric_meta.items():
        i = meta['index']
    ...
    pred_x = tree_pred_expr(tree, x_vars, 0); pred_y = tree_pred_expr(tree, y_vars, 0)
